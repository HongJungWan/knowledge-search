...
- **남은 항목**: #5 평가 상시화(모델 불필요, 회귀 게이트) → #2 라우팅(품질 무영향 지연 최적화) → #7 RRF/FTS 튜닝 → #6 운영 경로(Bedrock+OpenSearch).
- **보류 근거**: #6 AWS 자격증명·OpenSearch 인프라 필요. #2/#7 은 품질 영향이 작아 후순위. 실데이터 평가셋 확보가 다음 큰 레버(합성 코퍼스의 한계가 #3 등에서 드러남).
