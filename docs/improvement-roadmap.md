...
- **#3 청킹 측정 완료(null)**: 합성 코퍼스에선 효과 없음 → 기본 OFF, 진짜 장문 이질문서용으로 보관. 진단상 잔여 비정형의 레버는 granularity가 아니라 **의미 변별** = #4 리랭킹.
- **다음(권장)**: **#4 리랭킹** — top-k 를 cross-encoder/LLM-judge 로 재정렬해 의미 유사 상태를 변별(코드 표면형 없는 비정형 대상). 리랭커 모델 필요(로컬 LLM via Ollama 또는 cross-encoder) → 모델 확보 후 착수. → 이후 #5 평가 상시화 → #6 운영 경로(Bedrock+OpenSearch, AWS).
- **보류 근거**: #4 리랭커 모델·#6 AWS 자격증명 필요. #2 라우팅은 측정상 품질 영향이 없어(MO inert 시 무해) 지연 최적화로 후순위.
